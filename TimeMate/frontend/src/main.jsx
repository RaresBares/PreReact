...
/* ===================== Config / Backend ===================== */
const API_BASE = import.meta.env.VITE_API_BASE || 'http://localhost:8301';
const RESTAURANT_ID = "string";   // ggf. ändern
...
async function apiFetch(path, { method = 'GET', body, headers } = {}) {
  ...
  if (!res.ok) {
    let detail = { code: 'http_error', message: `HTTP ${res.status}` };
    try {
      const j = await res.json();
      detail = j?.payload || detail;
    } catch {}
...
// Versuche Settings zu laden; wenn Restaurant fehlt → registrieren (mit zufälligem username), dann erneut laden
async function loadSettingsWithAutoRegister() {
  // 1) direkter Versuch
  try {
    const d = await apiFetch(`/settings/${RESTAURANT_ID}`);
    return d?.payload;
  } catch (e) {
    if (e?.status !== 404) throw e;
  }
  // 2) registrieren (username einzigartig machen)
  const rand = Math.random().toString(36).slice(2, 8);
  await apiFetch(`/restaurants/register`, {
    method: 'POST',
    body: {
      id: RESTAURANT_ID,
      username: `ui-${RESTAURANT_ID}-${rand}`,
      display_name: 'TimeMate Demo',
    },
  }).catch(async (err) => {
    // Falls Username-Kollision o.ä.: noch ein Versuch mit anderem username
    if (err?.status === 409) {
      ...
      await apiFetch(`/restaurants/register`, {
        ...
        body: {
          id: RESTAURANT_ID,
          username: `ui-${RESTAURANT_ID}-${rand2}`,
          display_name: 'TimeMate Demo',
        },
      });
    } else {
  ...
  // 3) Settings erneut holen
  const d2 = await apiFetch(`/settings/${RESTAURANT_ID}`);
  return d2?.payload;
}

/* ===================== Helpers (Frontend) ===================== */
function toMin(hm){ const [h,m] = hm.split(':').map(Number); return h*60+m }
const combineDateTime = (baseDate, hour, minute) => { const d = new Date(baseDate); d.setHours(hour, minute, 0, 0); return d; };
...
function App(){
  ...
  const [globalFatal, setGlobalFatal] = useState('');

  const [settings, setSettings] = useState(null);
  const [holidays, setHolidays] = useState([]);
  const [openingWindows, setOpeningWindows] = useState([
    { start: toMin('08:00'), end: toMin('12:00') },
    { start: toMin('13:00'), end: toMin('18:00') },
  ]);
  const [minLeadMinutes, setMinLeadMinutes] = useState(120);
  const minLeadHours = Math.ceil(minLeadMinutes / 60);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const s = await loadSettingsWithAutoRegister();
        if (!alive) return;
        setSettings(s);
        setShowMap(!!s?.has_map);
        setOptions(Array.isArray(s?.extras) ? s.extras : []);
        setOpeningWindows((s?.opening_windows && s.opening_windows.length)
          ? s.opening_windows
          : openingWindows
        );
        setHolidays(Array.isArray(s?.holidays) ? s.holidays : []);
        setMinLeadMinutes(Number(s?.min_lead_minutes || 120));
      } catch (e) {
        setGlobalFatal(`Konnte Einstellungen nicht laden: ${e?.message || e}`);
      } finally {
  ...
  const applyMinute = (delta) => {
    ...
    const snapped = Math.round(nextMinute/15)*15 % 60;
    setHour(nextHour);
    setMinute(snapped);
  };
  ...
  const leadBound = (mins=minLeadMinutes) => new Date(Date.now() + mins*60*1000);
  const isWithinOpening = (when) => {
    const t = when.getHours()*60 + when.getMinutes();
    return (openingWindows || []).some(w => t >= Number(w.start||0) && t < Number(w.end||0));
  };
  ...
  const meetsLead   = dt >= leadBound(minLeadMinutes);
  ...
  const openConfirm = useCallback(() => {
    ...
    setShowConfirm(true);
  }, [meetsLead, businessOk, holiday, showMap, selectedSeat, minLeadHours]);

  if (globalFatal) {
    return (
      <main className="app-shell pt-nav">
        <div className="alert alert-danger m-3">{globalFatal}</div>
      </main>
    );
  }
  ...
  return (
    <>
      ...
      <nav className="navbar navbar-dark fixed-top nav-veil">
        <div className="app-shell d-flex align-items-center justify-content-between">
          ...
          <div className="d-flex align-items-center gap-2">
            <a className="btn btn-light btn-sm" href={HOME_HREF}>Home</a>
          </div>
        </div>
      </nav>
      ...
      {errorMsg && (
        <div className="alert-container">
          <div className="alert alert-danger py-2 px-3 m-0 d-flex justify-content-between align-items-center">
            <span>{errorMsg}</span>
            <button type="button" className="btn-close btn-close-white" aria-label="Close" onClick={()=>setErrorMsg('')}/>
          </div>
        </div>
      )}
      ...
      <main className={`app-shell pt-nav ${errorFx?'error-shake':''}`}>
        {globalLoading && (
          <div className="surface rounded-4 p-4 mb-4">
            <span className="spinner-border spinner-border-sm me-2" /> Lade Einstellungen…
          </div>
        )}
        ...
        <section className="section h-40dvh reveal rounded-4 p-4 mb-4 surface">
          ...
          <div className="time-matrix subtle">
            ...
            <button className="time-btn" onClick={()=>applyHour(-1)} disabled={!canStepMinutes(-60)} aria-label="Stunde -1">-1</button>
            <div className="time-colon down-spacer"></div>
            <button className="time-btn" onClick={()=>applyMinute(-15)} disabled={!canStepMinutes(-15)} aria-label="Minute -15">-15</button>
          </div>
            ...
            <div className="row g-2">
              {options.map(o=>(
                <div className="col-6 col-md-4" key={o}>
                  <label className={(selected.has(o)?'chip chip-active':'chip')+' w-100 text-center option has-tip'} data-tip={`Info zu "${o}"`}>
                    <input type="checkbox" className="d-none" checked={selected.has(o)} onChange={()=>toggleOption(o)} />
                    <span>{o}</span>
                  </label>
                </div>
              ))}
        ...
        <section className="section reveal rounded-4 p-4 mb-5 surface">
          ...
          <div className="d-flex justify-content-end mt-3">
            <button className="btn btn-light btn-sm" onClick={openConfirm} disabled={showMap && !selectedSeat}>
              Bestätigen
            </button>
          </div>
        </section>
      ...
      <ReservationConfirm
        ...
        summary={{
          restaurantId: RESTAURANT_ID,
          date: fullDate,
          isoDateTime: combineDateTime(baseDate, hour, minute).toISOString(),
          time: `${hh}:${mm}`,
          seatLabel: showMap ? (selectedSeat?.label || '—') : '—',
          seatId: showMap ? (selectedSeat?.id || null) : null,
          extras: [...selected],
          peopleDefault: 2,
        }}
      />
